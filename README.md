...
- **最新翻訳表示**: 拡張機能のポップアップに最新の翻訳結果を自動表示
- **翻訳履歴**: 過去50件の翻訳を保存・確認可能
- **ダークモード対応**: システムテーマに自動対応
- **カスタマイズ可能**: 自動ポップアップ表示のON/OFF切り替えなど
...
- **ブラウザ**: Google Chrome 88以降 / Microsoft Edge 88以降 (Chromium版)
...
4. **結果表示**: 
   - ページ上にポップアップが即座に表示
   - 拡張機能のポップアップが自動的に「最新」タブに切り替わり、翻訳結果が表示される（設定でONの場合）

### 翻訳結果の確認方法

翻訳結果は以下の方法で確認できます：

1. **ページ上のポップアップ**: 選択箇所の近くに表示される一時的なポップアップ
2. **拡張機能の「最新」タブ**: 拡張機能アイコンをクリックすると、最新の翻訳結果を確認可能
3. **翻訳履歴**: 過去50件の翻訳履歴を「履歴」タブで確認可能（最新のエントリは青色でハイライト表示）
...
- **翻訳完了時にポップアップを自動表示**: 翻訳が完了したら自動的に拡張機能のポップアップを開く（デフォルト: ON）

接続テストボタンで、LM Studioとの接続を確認できます。

### 便利な機能

- **コピー機能**: 「最新」タブや履歴から翻訳結果を素早くクリップボードにコピー
- **処理時間表示**: 各翻訳の処理時間を確認可能
- **ダークモード対応**: システムのテーマ設定に自動対応
